...
static void gsm_to8Bit(unsigned char in[], unsigned char out[], int len, int header_len);
...
static void gsm_convertNumber(char *res, char *dso, int toa)
{
	...
	} else if((toa & 0xD0) == 0xD0) { //Alphanumeric, (coded according to GSM TS03.38 7-bit default alphabet)
		res[0] = '\0';
		dso_len = strlen(dso);
		tmp = malloc(dso_len*2 + 1);
		memset(tmp,0,dso_len*2 + 1);
		gsm_string2byte((unsigned char*)tmp, dso, dso_len);
		gsm_to8Bit((unsigned char*)tmp, (unsigned char*)res, strlen(tmp), 0);
		free(tmp);
	} else {
		res[0] = '\0';
...
//Freedom Modify 2012-10-26 13:46
//Fix up receive long SMS error,The TP-UD of long SMS contain TP-UDHI (Header id)
#if 0
static void gsm_to8Bit(unsigned char in[], unsigned char out[], int len)
{
	int i = 0;
...
static void gsm_to8Bit(unsigned char in[], unsigned char out[], int len, int header_len)
{
	...
	int start = (header_len == 0) ? 0 : (header_len + header_len/7 + 1);

	//for(j=0,k=0; j < len; j++,k++) {
	for(j=start,k=0; j < len; j++,k++) {
		if(tmp[j] < 128) {
			if(tmp[j] == 0x1b && j+1 < len) {  //Special character, two bytes mean one character.
				switch(tmp[j+1]) {
				case 0x0A: out[k] = 0x000C; j++; break;  //	FORM FEED
...
//Freedom Modify 2013-07-30 18:33 decode 7bit data failed.
static void gsm_to8Bit(unsigned char in[], unsigned char out[], int len, int header_len)
{
//	int i = 0;
	unsigned char* tmp;
	tmp = (unsigned char*)malloc(len*2 + 1);
...
*/
...
#if 1
	int nSrc; // source char counter
	int nDst; // destination char counter
	int nByte; //current index (0-6)
	unsigned char nLeft; // last redundancy byte.

	unsigned char *pSrc = in;
	unsigned char *pDst = tmp;
	...
	// Put source data 7 bytes group, decode 8 bytes of 8 bit.
	while (nSrc < nSrcLen) {
...
int gsm_decode_pdu(struct gsm_modul *gsm, char *pdu, struct gsm_sms_pdu_info *pdu_info)
{
	...
	if (pdu_info->tp_oa.len > 0) {
		...
		memset(sender_number_dso,0,sizeof(sender_number_dso));
...
//	gsm->debug = 1;
	...
	if (udhi) {
		...
		//Freedom Add 2013-07-04 19:24. Skip long SMS flag(7bit don't do that), or others(I think)
		//If need process receive long SMS,Please modify here.
		if (pdu_info.tp_dcs & 0x08) { // Not 7BIT
			if(header_len*2 < sizeof(pdu_info.tp_ud)) {
				char tmp[sizeof(pdu_info.tp_ud)];
				memcpy(tmp,pdu_info.tp_ud,sizeof(tmp));
				memset(pdu_info.tp_ud,0,sizeof(pdu_info.tp_ud));
				memcpy(pdu_info.tp_ud, tmp + header_len*2, sizeof(pdu_info.tp_ud) - header_len*2);
			}
		}

		if (gsm->debug) {
			gsm_message(gsm, "Header Length = %d\n", header_len);
		}
	}
	...
	gsm_string2byte(user_data_7bit, pdu_info.tp_ud, pdu_info.tp_udl);

#if 0
	{
		int i;
		for(i=0; i<pdu_info.tp_udl; i++) {
			printf("%X,",user_data_7bit[i]);
		}
		printf("\n");
	}
#endif

	//Freedom Modify 2013-01-22 16:44 Fix up receive DCS is 0x09 messy code
	...
	if (pdu_info.tp_dcs & 0x08) {
		...
		//gsm_to8Bit(user_data_7bit, user_data_8bit, pdu_info.tp_udl, header_len);
		gsm_to8Bit(user_data_7bit, user_data_8bit, strlen(pdu_info.tp_ud)/2, header_len);
	}
...
static int gsm_encode_pdu(const char* SCA, const char* TPA, char TP_PID, char TP_DCS, const char* TP_UD, char* pDst,  int csms, unsigned char flag, unsigned char smscount, unsigned char smssequence)
{
	int nLength; 
	int nDstLength; 	
	...
	if(TP_DCS == GSM_7BIT) {
		...
		nLength = gsmEncode7bit(TP_UD, next, nLength+1, savelen);
		...
		leng_ucs = myUTF8_to_UNICODE(out, (unsigned char*)TP_UD,nLength);

		*savelen = gsmBytes2String_pdu(out, next,leng_ucs);
		nLength = *savelen + 4;
		...
		*savelen = gsmEncode8bit(TP_UD, next, nLength);
		nLength = *savelen + 4; 
...
int gsm_encode_pdu_ucs2(const char* smsc, const char* dest, char* sms_data, const char* coding,char* pdu, int csms, unsigned char flag, unsigned char smscount, unsigned char smssequence)
{
	char mesg[1024];
	...
	return gsm_encode_pdu(smsc == NULL ? "" : smsc , dest, 0, TP_DCS, mesg, pdu, csms, flag, smscount, smssequence);
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* 
//ASCII, Not need decode 
AT+CSCS="GSM"
OK
AT+CUSD=1,"*566#"
OK 
+CUSD: 2, "Your account balance is BDT 9.95 valid till 03/08/13. Get GP-GP 40 minutes ", 15 
OK
...
AT+CUSD=1,"*566#"
OK 
+CUSD: 2, "", 15 
*/

static int is_need_decode(char *inbuf, int inbuflen)
{
	int i;
	...
	if(inbuflen&1) { //If inbuflen is odd number, I think is ASCII code.
		return 0;
	}
...
int decode_ussd(char* inbuf, int inbuflen, char* outbuf, int outbuflen)
{
	char* bytes;
	int ret = 0;
	int byteslen = 0;
	...
	if(!is_need_decode(inbuf,inbuflen)) {
		return -1;
	}

	memset(outbuf,0,outbuflen);

	bytes = malloc(inbuflen/2+1);
	memset(bytes,0,inbuflen/2+1);

	for (i = 0; i < inbuflen/2; i++) {
		bytes[i] = (char)gsm_hex2int(&inbuf[i*2], 2);
	}
	byteslen = i;

	ret = code_convert("UCS-2","UTF-8",bytes,byteslen,outbuf,outbuflen);
