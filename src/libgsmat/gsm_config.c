...
struct at_cmd_t at_cmds[] = {
	...
	{AT_CHECK_SMSC,              "AT_CHECK_SMSC",              "+CSCA:"},
	...
	{AT_SIM_DECTECT,             "AT_SIM_DECTECT",             "AT"},/*for auto check sim status, Makes Add 2012-7-12 16:08*/
...
#ifdef AUTO_SIM_CHECK
	{AT_SIM_INSERT,              "AT_SIM_INSERT",              "Card insert"},/*for auto check sim status, Makes Add 2012-7-12 16:08*/
	{AT_SIM_REMOVE,              "AT_SIM_REMOVE",              "Card removed"},
	{AT_SIM_CFUN_MINI,           "AT_SIM_CFUN_MINI",           "AT+CFUN=0"},
	{AT_SIM_CFUN_FULL,           "AT_SIM_CFUN_FULL",           "AT+CFUN=1"},
#endif //AUTO_SIM_CHECK
	...
	{AT_CBAND,                   "AT_CBAND",                   "AT+CBAND=\"$BAND\""}, //Freedom Add for GSM band binding 2013-03-13 09:23 
...
static int get_coverage_and_ber(int module_id, char* h, int cmds_id)
{
	char buf[1024];
	...
	return coverage | (ber << 8);
}

int get_coverage1(int module_id, char* h)
{
	return get_coverage_and_ber(module_id,h, AT_GET_SIGNAL1);
}

int get_coverage2(int module_id, char* h)
{
	return get_coverage_and_ber(module_id,h, AT_GET_SIGNAL2);
}

char* get_setvol_str(int module_id, int value, char* buf, int len)
{
	char temp[1024];

	buf[0] = '\0';

	if(!str_check_symbol(get_at(module_id, AT_CLVL),temp,sizeof(temp)))
		return NULL;

	if(!str_replace(temp,temp,sizeof(temp),"$VOLUME_LEVEL","%d"))
		return NULL;

	snprintf(buf,len,temp,value);

	return buf;
}

char* get_setmic_str(int module_id, int value, char* buf, int len)
{
	char temp[1024];

	buf[0] = '\0';

	if(!str_check_symbol(get_at(module_id, AT_CMIC),temp,sizeof(temp)))
		return NULL;

	if(!str_replace(temp,temp,sizeof(temp),"$MICROPHONE_LEVEL","%d"))
		return NULL;
...
////////////////////////////////////////////////////////////
char* get_band_str(int module_id, const char* band, char* buf, int len)
{
	char temp[1024];
	buf[0] = '\0';

	if(!str_check_symbol(get_at(module_id,AT_CBAND),temp,sizeof(temp)))
		return NULL;
	
	if(!str_replace(temp,temp,sizeof(temp),"$BAND","%s"))
		return NULL;
	
	snprintf(buf,len,temp,band);
