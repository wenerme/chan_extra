...
#define DEFAULT_SYNC_SMS 20

//For "gsm send sms" and "gsm send pdu" CLI command,use to destination number max length.
#define GSM_MAX_PHONE_NUMBER 64

//For "gsm send sms" CLI command,use to SMS max length.
#define GSM_MAX_SMS_LENGTH 160*2   //A little insurance, multiply by 2.  Freedom 2013-03-26 09:21

//For "gsm send pdu" CLI command,use to PDU code max length.
#define GSM_MAX_PDU_LENGTH 176*2   //A little insurance, multiply by 2.  Freedom 2013-03-26 09:21


/* No more than 128 scheduled events */
#define MAX_SCHED	512//128   //Freedom Modify
...
enum EVENT_DEFINE {
	...
	GSM_EVENT_SIM_NOT_INSERTED,		/* SIM not inserted */
...
//////////////////////////////////////////////////////////////////////////////////
	...
	GSM_STATE_SIM_NOT_INSERT,
	GSM_STATE_SIM_CFUN_MINI,
	GSM_STATE_SIM_CFUN_FULL,
	GSM_STATE_IMSI_REQ,
	GSM_STATE_INIT_0,
	GSM_STATE_INIT_1,
	GSM_STATE_INIT_2,
	...
	GSM_STATE_BAND_BINDING,
	GSM_STATE_SELECT_OPERATOR,
	...
	GSM_STATE_SAVE_PARAM,
	GSM_STATE_NET_NAME_REQ,

	
	GSM_STATE_READY,	//Ready state. Init finish
	GSM_STATE_CALL_INIT,
	GSM_STATE_CALL_MADE,
...
struct outbound_statistics {
	unsigned int outbound_count;
	unsigned int duration_count;
	unsigned int answer_count;
	unsigned int cancel_count;
	unsigned int busy_count;
	unsigned int noanswer_count;
	unsigned int nodialtone_count;
	unsigned int nocarrier_count;
	unsigned char outbounded;
...
struct gsm_modul {
	...
	int oldstate;		/* Old state of D-channel */
...
#endif //GSM0710_MODE
	unsigned char init_sucess;
	unsigned char atz_timeout;
	...
	//Freedom Add for "gsm send syncat" 2013-03-13 14:15
	char rec_at[1024];
	int syncat_stat;
	...
	//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
	struct tx_queue* txq[1024];
	int txqi_a;				//Tx Queue index A.
	int txqi_b;				//Tx Queue index B.
	unsigned char have_report;	//Send AT command then has GSM module report.
	unsigned char wait_report;	//Send AT command then need wait GSM module report.
	time_t tq_sendtime;			//TX Queue Send time (time(NULL))
	int tq_sendcount;			//Tx Queue Send counter.
#endif

	//Freedom Add 2013-09-12 10:12
	struct outbound_statistics* pos;
};
...
extern int decode_ussd(char* inbuf, int inbuflen, char* outbuf, int outbuflen);
...
int gsm_encode_pdu_ucs2(const char* SCA, const char* TPA, char* TP_UD, const char* code,char* pDst, int csms, unsigned char flag, unsigned char smscount, unsigned char smssequence);
