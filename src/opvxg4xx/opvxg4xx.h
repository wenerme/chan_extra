...
/*
	Virtual TTY Mode   -Freedom
Have 2 Modes:
1. TTY mode only(only use by /dev/ttyGSM*, Asterisk can't use this channel), need define VIRTUAL_TTY, Don't define GSM0710_MODE, and need change load driver module use 'insmod opvxg4xx ttymode=1,2,...'
2. TTY mode and gsm0710 mode(Asterisk and /dev/ttyGSM* use at same time, It's multiplexing), need define VIRTUAL_TTY and GSM0710_MODE, and need set /etc/asterisk/extra_channels.conf 'tty=on'

*/

#ifdef GSM0710_MODE
#ifndef VIRTUAL_TTY
#error "If you define GSM0710_MODE,Please define VIRTUAL_TTY too !!"
#endif  //VIRTUAL_TTY
#endif //GSM0710_MODE
