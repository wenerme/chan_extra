...
#define OPVXG4XX_SPAN_GET_SIM		_IOR(OPVXG4XX_CODE, 20,unsigned char)
...
//Freedom Add 2013-05-25 14:57
//PDD stands for Post Dialing Delay, which is the time between the start of the call and the moment the phone of the called party starts ringing.
...
struct extra_gsm {
	...
	ast_mutex_t sms_lock;
	ast_cond_t sms_cond;
	ast_mutex_t sms_mutex;
	int sms_stat;


	ast_mutex_t at_mutex;
	ast_cond_t at_cond;
	...
	//Freedom Add for GSM band binding 2013-03-13 09:23
	char band[64];
	...
	struct outbound_statistics os;
};

static struct extra_gsm gsms[NUM_SPANS];

#ifdef TX_QUEUE
static void txq_lock(int span)
{
	if(span >=1 && span <= NUM_SPANS) {
		ast_mutex_lock(&gsms[span-1].txq_lock);
	}
}

static void txq_unlock(int span)
{
	if(span >=1 && span <= NUM_SPANS) {
		ast_mutex_unlock(&gsms[span-1].txq_lock);
	}
}
#endif //TX_QUEUE

//Freedom Add 2013-05-25 14:57
static unsigned char already_init[NUM_SPANS] = {0};
...
static const char *event2str(int event)
{
	...
	if ((event < (ARRAY_LEN(events))) && (event > -1))
		return events[event];
	snprintf(buf, sizeof(buf), "Event %d", event); /* safe */
	return buf;
}
...
{ 
	// Freedom Modify 2012-11-14 15:59 
	// Modify execute CLI commmand "core reload" can't use gsm bug. 
...
static int extra_hangup(struct ast_channel *ast)
{
	...
	/////////////////////////////////////////////////
	if(idx == SUB_REAL) {
		if(p->gsm->dl.dial_time) {

			p->gsm->dl.dial_count++;
			p->gsm->dl.hangup_time = sys_uptime();

			if(p->gsm->dl.ring_time) {
				time_t tmp;
				tmp = p->gsm->dl.ring_time - p->gsm->dl.dial_time;
				if(tmp > 0) {
					p->gsm->dl.pdd_time += tmp;
					p->gsm->dl.pdd_count++;
				}
			}

			if(p->gsm->dl.answer_time) {
				time_t tmp;
				tmp = p->gsm->dl.hangup_time - p->gsm->dl.answer_time;
				if(tmp > 0) {
					p->gsm->dl.acd_time += tmp;
					p->gsm->dl.acd_count++;
					//Freedom Add 2013-09-12 10:12
					p->gsm->os.duration_count += tmp;
				}
				p->gsm->dl.answer_count++;
			}
		}
		p->gsm->dl.dial_time = 0;
		p->gsm->dl.ring_time = 0;
		p->gsm->dl.answer_time = 0;
	}
	/////////////////////////////////////////////////
	...
	if(p->sig == SIG_GSM_EXTRA){
		...
		//Freedom Modify 2013-05-20 16:00 Fix up if gsm income call answer after, then receive SMS none CID. 
		if( idx == SUB_REAL || idx == SUB_CALLWAIT || idx == SUB_THREEWAY ) {
			p->cid_num[0] = '\0';
			p->cid_name[0] = '\0';
		}
	}
...
#if (ASTERISK_VERSION_NUM >= 110200)
...
#elif (ASTERISK_VERSION_NUM >= 101200)
			...
			if (chan_sig == SIG_GSM_EXTRA) {
				...
				if (span >= NUM_SPANS) {
					...
					if (!matchesdchan) {
						...
						//Freedom Add 2013-05-25 14:57
						if(0 == already_init[span]) {
							already_init[span] = 1;
							gsms[span].dl.dial_count = 0;
							gsms[span].dl.answer_count = 0;
							gsms[span].dl.dial_time = 0;
							gsms[span].dl.ring_time = 0;
							gsms[span].dl.answer_time = 0;
							gsms[span].dl.hangup_time = 0;
							gsms[span].dl.pdd_time = 0;
							gsms[span].dl.pdd_count = 0; 
							gsms[span].dl.acd_time = 0;
							gsms[span].dl.acd_count = 0;

							gsms[span].os.outbound_count = 0;
							gsms[span].os.duration_count = 0;
							gsms[span].os.answer_count = 0;
							gsms[span].os.cancel_count = 0;
							gsms[span].os.busy_count = 0;
							gsms[span].os.noanswer_count = 0;
							gsms[span].os.nodialtone_count = 0;
							gsms[span].os.nocarrier_count = 0;
							gsms[span].os.outbounded = 0;
							gsms[span].os.answered = 0;
						}

						gsms[span].debug_at_flag = conf->gsm.debug_at_flag;
#ifdef SIMULATE_DIAL
						gsms[span].simulatedial = conf->gsm.simulatedial;
#endif //SIMULATE_DIAL

						//Freedom Add for anonymous call 2013-08-15 09:37
						gsms[span].anonymouscall = conf->gsm.anonymouscall;
						gsms[span].vol = conf->gsm.vol;
						gsms[span].mic = conf->gsm.mic;
						gsms[span].atz_timeout = conf->gsm.atz_timeout == 0 ? 2 : conf->gsm.atz_timeout ;
						//Freedom Add for GSM band binding 2013-03-13 09:23 
						strncpy(gsms[span].band,conf->gsm.band,sizeof(gsms[span].band));
						//Jason Add dial prefix 2013-5-9
						strncpy(gsms[span].dialprefix,conf->gsm.dialprefix,sizeof(gsms[span].dialprefix));
...
static int gsm_hangup_all(struct extra_pvt *p, struct extra_gsm *gsm)
{
	...
	do {
		...
		for (x = 0; x < SUB_SUM; x++) {
			...
			if (p->subs[x].owner) {
#if (ASTERISK_VERSION_NUM >= 10601)
				//Freedom Modify 2013-11-01 17:27 Fix up Dial() $DIALSTATUS set to CHANUNAVAIL
		...
		const struct ast_channel *requestor)
{
	...
	//Freedom Modify 2013-05-15 16:43 for fix up when channel call active,receive SMS,then run dialplan asterisk dump. 
/*		
#if (ASTERISK_VERSION_NUM >= 10800)
...
*/
#if (ASTERISK_VERSION_NUM >= 10800)
	chn = ast_channel_alloc(0, state, NULL, NULL, pvt->accountcode, pvt->exten, pvt->context, NULL, pvt->amaflags, "EXTRA-SMS/%s", ast_str_buffer(chan_name));
#else  //(ASTERISK_VERSION_NUM >= 10800)
#if (ASTERISK_VERSION_NUM > 10444)
	chn = ast_channel_alloc(0, state, NULL, NULL, pvt->accountcode, pvt->exten, pvt->context, pvt->amaflags, "EXTRA-SMS/%s", chan_name->str);
#else  //(ASTERISK_VERSION_NUM > 10444)
	chn = ast_channel_alloc(0, state, NULL, NULL, pvt->accountcode, pvt->exten, pvt->context, pvt->amaflags, "EXTRA-SMS/%s", b2);
#endif //(ASTERISK_VERSION_NUM > 10444)
#endif //(ASTERISK_VERSION_NUM >= 10800)
	...
	//Freedom Modify 2013-07-04 11:58 Don't cover pvt->owner, fix up when channel call active,receive SMS,then process module DTMF asterisk dump. 
	//pvt->owner = chn;
...
static void *gsm_dchannel(void *vgsm)
{
	...
	for (;;) {
		...
		//Freedom Move code out side "if (!res) {" 2013-08-12 14:31
		if(gsm->gsm_init_flag == 0) {
			gsm->gsm_reinit++;
			if(gsm->gsm_reinit%10 == 0) { //About 15 second power to GSM Module if GSM Module can't detect.
				ioctl(gsm->fd, OPVXG4XX_SPAN_INIT, 1);
				ast_log(LOG_NOTICE, "GSM reset power of module on D-channel of span %d(Detect module)\n", gsm->span);
				sleep(1); //Wait module start.
			} else {
				ast_log(LOG_NOTICE, "GSM detect module on D-channel of span %d(Detect module)\n", gsm->span);
//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
				set_report(gsm->dchan);
#endif
		...
		if (!res) {
			...
			if(gsm->gsm_init_flag == 0) {
				gsm->gsm_reinit++;
				if(gsm->gsm_reinit%2 == 0) { //About 3 second Sent 'AT' to GSM Module if GSM Module can't report 'OK'.
					if(gsm->gsm_reinit%6 == 0) { //About 9 second power to GSM Module if GSM Module can't detect.
						ioctl(gsm->fd, OPVXG4XX_SPAN_INIT, 1);
						ast_log(LOG_NOTICE, "GSM reset power of module on D-channel of span %d(Detect module)\n", gsm->span);
					} else {
						ast_log(LOG_NOTICE, "GSM detect module on D-channel of span %d(Detect module)\n", gsm->span);
...
//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
				if(!is_active_report(gsm->dchan) && is_normal_reply(gsm->dchan))
					set_report(gsm->dchan);
#endif //TX_QUEUE


				if(gsm->dchan->syncat_stat) {
#ifdef TX_QUEUE
					if(!is_active_report(gsm->dchan) && is_normal_reply(gsm->dchan)) {
						gsm->dchan->syncat_stat = 0;
						strncpy(gsm->dchan->rec_at, gsm->dchan->at_pre_recv, sizeof(gsm->dchan->rec_at));
						ast_cond_signal(&gsm->at_cond);
					}
#else  //TX_QUEUE
					gsm->dchan->syncat_stat = 0;
					strncpy(gsm->dchan->rec_at, gsm->dchan->at_pre_recv, sizeof(gsm->dchan->rec_at));
...
#ifdef TX_QUEUE
		{
			gsm_event* report_event;
			if(NULL != (report_event = process_report(gsm->dchan)) ) {
				e = report_event;
			}
		}
#endif
		...
		if (e) {
...
#endif //SIMULATE_DIAL
				if(gsm->gsm->init_sucess)
					gsm->gsm->init_sucess=0;
			...
			//Freedom Add 2013-05-25 14:57
			////////////////////////////////////////////////////
			case GSM_EVENT_MO_RINGING:
				chanpos = e->answer.channel;
				if (chanpos >= 0) {
					ast_mutex_lock(&gsm->pvt->lock);
					gsm->dl.ring_time = sys_uptime();
					ast_mutex_unlock(&gsm->pvt->lock);
				}
				break;
			////////////////////////////////////////////////////
			...
			{
				unsigned int sim_status = 0;
				gsm->gsm->init_sucess = 0;
				ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_GET_SIM, &sim_status);
				if(sim_status&(1<<(gsm->span-1))) {

					gsm->dchanavail &= ~DCHAN_UP;
					gsm->dchanavail |= DCHAN_NO_SIM;
					ast_log(LOG_NOTICE, "The GSM SIM not inserted\n");
					if (!gsm_is_up(gsm)) {
						gsm->resetting = 0;
						/* Hangup active channels and put them in alarm mode */
						struct extra_pvt *p = gsm->pvt;
			...
			{
				unsigned int sim_status = 0;
				ast_log(LOG_NOTICE, "The GSM SIM inserted %d\n",gsm->gsm->init_sucess);
				gsm->gsm_init_flag = 0;
				gsm->gsm_reinit = 0;
				memset(gsm->gsm->manufacturer,0,sizeof(gsm->gsm->manufacturer));
				memset(gsm->gsm->sim_smsc,0,sizeof(gsm->gsm->sim_smsc));
				memset(gsm->gsm->model_name,0,sizeof(gsm->gsm->model_name));
				memset(gsm->gsm->revision,0,sizeof(gsm->gsm->revision));
				memset(gsm->gsm->imei,0,sizeof(gsm->gsm->imei));
				memset(gsm->gsm->imsi,0,sizeof(gsm->gsm->imsi));
				ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_GET_SIM, &sim_status);
				if(sim_status&(1<<(gsm->span-1))) {
					gsm->gsm->init_sucess = 0;
				} else {
...
#else  //(ASTERISK_VERSION_NUM >= 110000)
					strncpy(c->exten, context_name, sizeof(c->exten));
#endif //(ASTERISK_VERSION_NUM >= 110000)
			...
			{
				ioctl(gsm->fd, OPVXG4XX_SPAN_INIT, 1);
				ast_log(LOG_NOTICE, "GSM reset power of module on D-channel of span %d(Send AT Command timeout and resend failed)\n", gsm->span);
				gsm->gsm_init_flag = 0;
				gsm->gsm_reinit = 0;
				sleep(1);	//Wait Module start
				gsm_module_start(gsm->dchan);
...
static int start_gsm(struct extra_gsm *gsm)
{
	...
	gsm->gsm = gsm->dchan;
	//Freedom Add 2013-09-12 10:12
	gsm->dchan->pos = &gsm->os;

	gsm->dchan->vol = gsm->vol;
	gsm->dchan->mic = gsm->mic;
	gsm->dchan->atz_timeout = gsm->atz_timeout;

	//Freedom Add for GSM band binding 2013-03-13 09:23 
	strncpy(gsm->dchan->band,gsm->band,sizeof(gsm->dchan->band));
	//Jason Add dial prefix 2013-5-9
	strncpy(gsm->dchan->dialprefix,gsm->dialprefix,sizeof(gsm->dchan->dialprefix));
#ifdef SIMULATE_DIAL
	gsm->dchan->simulatedial = gsm->simulatedial;
#endif //SIMULATE_DIAL

	//Freedom Add for anonymous call 2013-08-15 09:37
	gsm->dchan->anonymouscall = gsm->anonymouscall;
...
static char* gsm_get_cdrtime(struct extra_gsm *egsm, char* buf, int buflen)
{
	memset(buf,0,buflen);

	if (egsm) {
		if(egsm->dl.answer_time > 0 ) {
			time_t now = sys_uptime();
			long  duration  =  (long)(now - egsm->dl.answer_time);
			if(duration > 0) {
...
static char* gsm_build_state(struct extra_gsm* egsm)
{
	...
	if(is_call_state(egsm->gsm->state)) {
		if(egsm->pvt) {
			if(egsm->pvt->cid_num[0] != '\0') {
				//struct ast_channel *c;
				//Freedom Modify 2013-05-15 16:43 for fix up when channel call active,receive SMS,then run dialplan asterisk dump.
				//c = egsm->pvt->owner;
				//c = egsm->pvt->subs[SUB_REAL].owner;
				char cdrtime[256];
				gsm_get_cdrtime(egsm,cdrtime,sizeof(cdrtime));
				snprintf(ret_str,1024,"%s Called from %s%s",gsm_state2str(egsm->gsm->state), egsm->pvt->cid_num, cdrtime);
				return ret_str;
			} else if(egsm->pvt->gsmcall && egsm->pvt->gsmcall->callednum[0] != '\0') {
				//struct ast_channel *c;
				//Freedom Modify 2013-05-15 16:43 for fix up when channel call active,receive SMS,then run dialplan asterisk dump.
				//c = egsm->pvt->owner;
				//c = egsm->pvt->subs[SUB_REAL].owner;
				char cdrtime[256];
				gsm_get_cdrtime(egsm,cdrtime,sizeof(cdrtime));
				snprintf(ret_str,1024,"%s Called to %s%s",gsm_state2str(egsm->gsm->state), egsm->pvt->gsmcall->callednum, cdrtime);
				return ret_str;
			}
		}
	}
...
static void show_span(int span, int fd)
{
	...
	long acd = 0;
	if(gsms[span].pvt) {
		ast_mutex_lock(&gsms[span].pvt->lock);
		if(gsms[span].dl.pdd_count > 0) {
			pdd = (long)((float)gsms[span].dl.pdd_time / (float)gsms[span].dl.pdd_count);
		}

		if(gsms[span].dl.acd_count > 0) {
			acd = (long)((float)gsms[span].dl.acd_time / (float)gsms[span].dl.acd_count);
		}

		if(gsms[span].dl.dial_count) {
			asr = (int)((float)gsms[span].dl.answer_count / (float)gsms[span].dl.dial_count * 100.0);
			if((asr <= 0) && gsms[span].dl.answer_count > 0) { //If answered.ASR minimum is 1.
				asr = 1;
			}
		}
		ast_mutex_unlock(&gsms[span].pvt->lock);
	}
	ast_cli(fd,"PDD: %ld\n",pdd);
	ast_cli(fd,"ASR: %d\n",asr);
	ast_cli(fd,"ACD: %ld\n",acd);
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	if(strcasecmp(argv[3],"all") == 0 ){
		for (span = 0; span < NUM_SPANS; span++ ) {
			if (gsms[span].gsm && gsms[span].dchan && gsms[span].dchannel) {
				ast_cli(fd, "---Start Span %d---\n", span + 1);
				show_span(span,fd);
				ast_cli(fd, "---End Span %d---\n", span + 1);
			}
		}
	} else {
		span = atoi(argv[3]);
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	if(ast_mutex_trylock(&gsm->ussd_mutex) != 0 ) {
		ast_cli(fd, "0:Sending USSD now on span %d\n",span);
		return _SUCCESS_;
	}

	gsm_send_ussd(gsm->gsm, (char*)argv[4]);
	ret = ast_cond_timedwait(&gsm->ussd_cond, &gsm->ussd_mutex, &ts);
	if(ret == 0) {
		if(gsm->ussd_received.return_flag) {
			//Freedom Modify 2013-05-13 14:51 for simple show USSD response.
			//ast_cli(fd, "1:Recive USSD sucess on span %d\n", span);
			//ast_cli(fd, "\tUSSD Responses:%d\n", gsm->ussd_received.ussd_stat);
			//ast_cli(fd, "\tUSSD Code:%d\n", gsm->ussd_received.ussd_coding);
			//ast_cli(fd, "\tUSSD Message:%s\n", gsm->ussd_received.text);

			ast_cli(fd, "1:Recive USSD on span %d,responses:%d,code:%d\n", span, gsm->ussd_received.ussd_stat, gsm->ussd_received.ussd_coding);
			ast_cli(fd, "Text:%s\n", gsm->ussd_received.text);
		} else {
	...
	//Freedom Modify 2013-05-13 14:51 for simple show USSD response.
	//Free memory. Malloc in gsm_send_ussd 
	if(gsm->dchan->ussd_info) {
		free(gsm->dchan->ussd_info);
		gsm->dchan->ussd_info = NULL;
	}
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	if(gsm->send_sms.mode == SEND_SMS_MODE_PDU) {
		char pdu[1024];		
		if(!gsm_encode_pdu_ucs2(gsm->send_sms.smsc, (char*)argv[4],(char*)argv[5], gsm->send_sms.coding, pdu,0,0,0,0)) {
			ast_cli(fd,"Encode pdu error\n");
			return _FAILURE_;
		}
	
		ast_mutex_lock(&gsm->lock);
		gsm_send_pdu(gsm->gsm, (char*)pdu, (char*)argv[5], id);
		ast_mutex_unlock(&gsm->lock);
	} else {
		if(!is_ascii((char*)argv[5])) {
			ast_cli(fd,"Only support ASCII on text mode\n");
			return _FAILURE_;
		}
		ast_mutex_lock(&gsm->lock);
		gsm_send_text(gsm->gsm, (char*)argv[4], (char*)argv[5], id);
		ast_mutex_unlock(&gsm->lock);
	}
...
#if (ASTERISK_VERSION_NUM > 10444)
static char *handle_gsm_send_sync_sms(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
#else  //(ASTERISK_VERSION_NUM > 10444)
static int handle_gsm_send_sync_sms(int fd,int argc, char **argv)
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	int span;
	struct extra_gsm* gsm;
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gsm send sync sms";
		e->usage =
			"Usage: gsm send sync sms <span> <destination> <message> <timeout> [id] \n"
			"       Send SMS on a given GSM span\n";
		return NULL;
	case CLI_GENERATE:
		return gsm_complete_span_5(a->line, a->word, a->pos, a->n);
	...
	timeout=atoi(argv[7] );
	span = atoi(argv[4]);
	id = argc >= 8 ? argv[8] : NULL;
	if (! is_dchan_span(span,fd) ) return _FAILURE_;

	gsm = &gsms[span-1];

	int phone_num_len = strlen(argv[5]);
	int sms_len = strlen(argv[6]);
	...
	if(ast_mutex_trylock(&gsm->sms_lock) == 0){
		ts.tv_sec = time(NULL) + timeout;
		ts.tv_nsec = 0;
		ast_mutex_lock(&gsm->sms_mutex);
		gsm->sms_stat=0;
		if(gsm->send_sms.mode == SEND_SMS_MODE_PDU) {
			char pdu[1024];		
			if(!gsm_encode_pdu_ucs2(gsm->send_sms.smsc, (char*)argv[5],(char*)argv[6], gsm->send_sms.coding, pdu,0,0,0,0)) {
				ast_mutex_unlock(&gsm->sms_mutex);
				ast_mutex_unlock(&gsm->sms_lock);
				if(!id)
					ast_cli(fd,"SPAN:%d SEND SMS TO PHONE %s ENCODE PDU FAIL\n",span,(char*)argv[5]);
				else
					ast_cli(fd,"ID:%s SPAN:%d SEND SMS TO PHONE %s ENCODE PDU FAIL\n",id,span,(char*)argv[5]);
				return _FAILURE_;
			}
			ast_mutex_lock(&gsm->lock);
			gsm_send_pdu(gsm->gsm, (char*)pdu, (char*)argv[6], id);
			ast_mutex_unlock(&gsm->lock);
		} else {
			if(!is_ascii((char*)argv[6])) {
				ast_mutex_unlock(&gsm->sms_mutex);
				ast_mutex_unlock(&gsm->sms_lock);
				ast_cli(fd,"Only support ASCII on text mode\n");
				return _FAILURE_;
			...
			ast_mutex_lock(&gsm->lock);
			gsm_send_text(gsm->gsm, (char*)argv[5], (char*)argv[6], id);
			ast_mutex_unlock(&gsm->lock);
		}
		ret = ast_cond_timedwait(&gsm->sms_cond,&gsm->sms_mutex,&ts);
		if(ret==0){
			if(gsm->sms_stat){
				if(!id)
					ast_cli(fd,"SPAN:%d SEND SMS TO PHONE:%s SUCCESSFULLY\n",span,(char*)argv[5]);
				else
					ast_cli(fd,"ID:%s SPAN:%d SEND SMS TO PHONE:%s SUCCESSFULLY\n",id,span,(char*)argv[5]);
			}else{
				if(!id)
					ast_cli(fd,"SPAN:%d SEND SMS TO PHONE:%s WAS FAILED\n",span,(char*)argv[5]);
				else
					ast_cli(fd,"ID:%s SPAN:%d SEND SMS TO PHONE:%s WAS FAILED\n",id,span,(char*)argv[5]);
			}
		}else{
			if(!id)
				ast_cli(fd,"SPAN:%d SEND SMS TO PHONE:%s TIMEOUT\n",span,(char*)argv[5]);
			else
				ast_cli(fd,"ID:%s SPAN:%d SEND SMS TO PHONE:%s TIMEOUT\n",id,span,(char*)argv[5]);
		}
		ast_mutex_unlock(&gsm->sms_mutex);
		ast_mutex_unlock(&gsm->sms_lock);
	} else {
		if(!id)
			ast_cli(fd,"SPAN:%d SEND SMS TO PHONE %s WAS USING\n",span,(char*)argv[5]);
		else
			ast_cli(fd,"ID:%s SPAN:%d SEND SMS TO PHONE %s WAS USING\n",id,span,(char*)argv[5]);
	}
	return _SUCCESS_;
}
...
#if (ASTERISK_VERSION_NUM > 10444)
static char *handle_gsm_send_sync_csms(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
#else  //(ASTERISK_VERSION_NUM > 10444)
static int handle_gsm_send_sync_csms(int fd,int argc, char **argv)
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	int span;
	int ret = 0;
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gsm send sync csms";
		e->usage =
			"Usage: gsm send sync csms <span> <destination> <message> <flag> <smscount> <smssequence> <timeout> [id] \n"
			"       Send Concatenated SMS on a given GSM span\n";
		return NULL;
	case CLI_GENERATE:
		return gsm_complete_span_5(a->line, a->word, a->pos, a->n);
	}
	...
	timeout = atoi(argv[10]);
	span = atoi(argv[4]);
	id = argc >= 11 ? argv[11] : NULL;
	if (! is_dchan_span(span,fd) ) return _FAILURE_;
	...
	flag = atoi(argv[7]);
	smscount = atoi(argv[8]);
	smssequence = atoi(argv[9]);
	...
	if(ast_mutex_trylock(&gsm->sms_lock) == 0){
		...
		if(gsm->send_sms.mode == SEND_SMS_MODE_PDU) {
			char pdu[1024];
			if(!gsm_encode_pdu_ucs2(gsm->send_sms.smsc, (char*)argv[5],(char*)argv[6], gsm->send_sms.coding, pdu,1,flag,smscount,smssequence)) {
				ast_mutex_unlock(&gsm->sms_mutex);
				ast_mutex_unlock(&gsm->sms_lock);
				if(!id)
					ast_cli(fd,"SPAN:%d SEND SMS TO PHONE %s ENCODE PDU FAIL\n",span,(char*)argv[5]);
			...
			ast_mutex_unlock(&gsm->sms_mutex);
			ast_mutex_unlock(&gsm->sms_lock);
			ast_cli(fd,"Send concatenated SMS can't support text mode, please use pdu mode\n");
			return _FAILURE_;
		}
		...
		if(ret == 0){
			if(gsm->sms_stat){
				if(!id)
					ast_cli(fd,"SPAN:%d SEND CONCATENATED SMS TO PHONE:%s SUCCESSFULLY\n",span,(char*)argv[5]);
				else
					ast_cli(fd,"ID:%s SPAN:%d SEND CONCATENATED SMS TO PHONE:%s SUCCESSFULLY\n",id,span,(char*)argv[5]);
			}else{
				if(!id)
					ast_cli(fd,"SPAN:%d SEND CONCATENATED SMS TO PHONE:%s WAS FAILED\n",span,(char*)argv[5]);
				else
					ast_cli(fd,"ID:%s SPAN:%d SEND CONCATENATED SMS TO PHONE:%s WAS FAILED\n",id,span,(char*)argv[5]);
			}
		} else {
			if(!id)
				ast_cli(fd,"SPAN:%d SEND CONCATENATED SMS TO PHONE:%s TIMEOUT\n",span,(char*)argv[5]);
			else
				ast_cli(fd,"ID:%s SPAN:%d SEND CONCATENATED SMS TO PHONE:%s TIMEOUT\n",id,span,(char*)argv[5]);
		}
		ast_mutex_unlock(&gsm->sms_mutex);
		ast_mutex_unlock(&gsm->sms_lock);
	} else {
		if(!id)
			ast_cli(fd,"SPAN:%d SEND CONCATENATED SMS TO PHONE %s WAS USING\n",span,(char*)argv[5]);
		else
			ast_cli(fd,"ID:%s SPAN:%d SEND CONCATENATED SMS TO PHONE %s WAS USING\n",id,span,(char*)argv[5]);
	}
...
#if (ASTERISK_VERSION_NUM > 10444)
static char *handle_gsm_send_sync_at(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
#else  //(ASTERISK_VERSION_NUM > 10444)
static int handle_gsm_send_sync_at(int fd,int argc, char **argv)
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	int span;
	struct extra_gsm* gsm;
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gsm send sync at";
		e->usage =
			"Usage: gsm send sync at <span> <AT Command> <timeout(ms)>\n"
			"       Send AT Command on a given GSM span\n";
		return NULL;
	case CLI_GENERATE:
		return gsm_complete_span_5(a->line, a->word, a->pos, a->n);
	...
	strncpy(at_command, argv[5], sizeof(at_command));

	timeout = atoi(argv[6]);
	if(timeout < 0) return _FAILURE_;

	ast_mutex_lock(&gsm->at_mutex);
	memset(gsm->dchan->rec_at, 0, sizeof(gsm->dchan->rec_at));
	gsm_test_atcommand(gsm->dchan, at_command);
	gsm->dchan->syncat_stat = 1;

	now = ast_tvnow();
	ts.tv_sec = now.tv_sec + timeout/1000;
	ts.tv_nsec = now.tv_usec * 1000 + (timeout%1000) * 1000;
	if( 0 != ast_cond_timedwait(&gsm->at_cond, &gsm->at_mutex, &ts) ) {
		ast_cli(fd,"TIMEOUT\n");
	} else {
		if(strlen(gsm->dchan->rec_at)) {
			ast_cli(fd,"%s",gsm->dchan->rec_at);
			memset(gsm->dchan->rec_at,0,sizeof(gsm->dchan->rec_at));
		}
	}
	//gsm->dchan->syncat_stat = 0;
	ast_mutex_unlock(&gsm->at_mutex);
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	case CLI_INIT:
		...
		e->usage =
			"Usage: gsm send pdu <span> <PDU codes> [id]\n"
	...
	if( pdu_len <= 0 ) {
		ast_cli(fd,"PDU message too short.\n");
		return _FAILURE_;
	} else if(pdu_len > GSM_MAX_PDU_LENGTH) {
		ast_cli(fd,"PDU message too long.\n");
		return _FAILURE_;
	}

	ast_mutex_lock(&gsm->lock);
	gsm_send_pdu(gsm->gsm, (char*)argv[4], NULL, id);
	ast_mutex_unlock(&gsm->lock);
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	strncpy(gsm->send_sms.smsc,argv[6],sizeof(gsm->send_sms.smsc));
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	strncpy(gsm->send_sms.coding,argv[6],sizeof(gsm->send_sms.coding));
	strncpy(gsm->gsm->sms_text_coding,gsm->send_sms.coding,strlen(gsm->send_sms.coding));
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	if(argc == 8)
		timeout=atoi(argv[7]);
	if(ast_mutex_trylock(&gsm->phone_lock)==0){
		gsm_set_check_phone_mode(gsm->gsm,1);
		ts.tv_sec = time(NULL) + timeout;
		ts.tv_nsec = 0;
		ast_mutex_lock(&gsm->check_mutex);
		ret=gsm_check_phone_stat(gsm->gsm,phone_number,hangup_mode,timeout);
		if(ret!=0){
		...
		ret=ast_cond_timedwait(&gsm->check_cond,&gsm->check_mutex,&ts);
		if(ret==0){
			switch(gsm->phone_stat){
				case SPAN_USING:
					ast_cli(fd,"SPAN:%d USING\n",span);
					break;
				case PHONE_CONNECT:
					ast_cli(fd,"PHONE:%s CONNECT\n",phone_number);
					gsm_set_check_phone_mode(gsm->gsm,0);
					break;
				case PHONE_RING:
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	ast_mutex_lock(&gsm->lock);
	if (ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_INIT, 0)==0) {
		gsm->gsm_init_flag=0;
		gsm->gsm_reinit=0;
		sleep(2);	//Wait Module start
		gsm_module_start(gsm->gsm);
...
#if 0
static void gsm_channel_reset(int span)
{
	ast_verbose("FreedomDebug:gsm_channnel_reset\n");
	if (gsm_is_up(&gsms[span-1])) {
		ast_verbose("FreedomDebug:gsm_is_up\n");
		/*gsm->resetting = 0;*/
		/* Hangup active channels and put them in alarm mode */
		struct extra_pvt *p = gsm->pvt;
		if (p) {
#if 0
			ast_verbose("FreedomDebug:pvt\n");
			if(p->gsm && p->gsm->gsm && p->gsmcall) {
				gsm_hangup(p->gsm->gsm, p->gsmcall, -1);
				gsm_destroycall(p->gsm->gsm, p->gsmcall);
				p->gsmcall = NULL;
			}
...
#endif //(ASTERISK_VERSION_NUM >= 110000)
#endif
				ast_verbose("FreedomDebug:ast_hangup\n");
				ast_hangup(p->owner);
			}
		}
	}
...
#if 1
static void gsm_channel_reset(struct extra_gsm* gsm)
{
	/*if (!gsm_is_up(gsm))*/ {
		...
		if (p) {
			...
			/*if (!p->gsm || !p->gsm->gsm || gsm_get_timer(p->gsm->gsm, GSM_TIMER_T309) < 0)*/ {
				...
				if (p->gsmcall) {
					if (p->gsm && p->gsm->gsm) {
						//ast_verbose("FreedomDebug:gsm_hangup\n");
						gsm_hangup(p->gsm->gsm, p->gsmcall, -1);
						gsm_destroycall(p->gsm->gsm, p->gsmcall);
						p->gsmcall = NULL;
					} else
				...
				//ast_verbose("FreedomDebug:gsm_hangup_all\n");
				//gsm_hangup_all(p, gsm);
...
*/
					...
					//ast_verbose("FreedomDebug:softhangup\n");
				}
				//ast_verbose("[%p,%p]\n",p->subs[SUB_REAL].owner,p->owner);
			}
			p->inalarm = 1;
			ast_mutex_unlock(&p->lock);
		}
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_STAT, &power_stat);
	if(power_stat) {
		//gsm_channnel_reset(span);
		gsm_channel_reset(&gsms[span-1]);
		if (ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_REMOVE, 0)==0) {
	...
	if(power_stat) {
		if (ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_REMOVE, 0)!=0) {
			...
			if (ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_INIT, 0)!=0) {
				...
				gsm_module_start(gsm->gsm);
				//gsm_channel_reset(&gsms[span-1]);
				ast_cli(fd, "Reset power on span %d success\n",span);
			}
		}
	} else {
		if (ioctl(gsm->gsm->fd, OPVXG4XX_SPAN_INIT, 0)!=0) {
...
#if (ASTERISK_VERSION_NUM > 10444)
static char * handle_gsm_show_statistics_outbound(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
#else  //(ASTERISK_VERSION_NUM > 10444)
static int handle_gsm_show_statistics_outbound(int fd,int argc, char **argv)
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	int span;
	...
	switch (cmd) {
	case CLI_INIT:
		e->command = "gsm show statistics outbound";
		e->usage = 
			"Usage: gsm show statistics outbound\n"
	...
	ast_cli(fd, "Span | Count | All Duration | Answerd | Cancel | Busy | No Answer | No Dialtone | No Carrier\n");
	for (span = 0; span < NUM_SPANS; span++) {
		struct extra_gsm* p = &gsms[span];
		if (p->gsm) {
			if (p->dchannel) {
				if(p->pvt) {
					ast_mutex_lock(&p->pvt->lock);
					ast_cli(fd, "%-5d| %-6d| %-13d| %-8d| %-7d| %-5d| %-10d| %-12d| %-9d\n", p->span,
							p->os.outbound_count, p->os.duration_count,p->os.answer_count, p->os.cancel_count,
							p->os.busy_count,p->os.noanswer_count, p->os.nodialtone_count, p->os.nocarrier_count);
					ast_mutex_unlock(&p->pvt->lock);
				}
			}
		}
	}
...
static struct ast_cli_entry extra_gsm_cli[] = {
	...
	AST_CLI_DEFINE(handle_gsm_send_sync_sms, "Send SMS and wait finish on a given GSM span"),
	AST_CLI_DEFINE(handle_gsm_send_sync_csms, "Send Concatenated SMS and wait finish on a given GSM span"),
	AST_CLI_DEFINE(handle_gsm_send_sync_at, "Send AT Command and wait finish on a given GSM span"),
	...
	AST_CLI_DEFINE(handle_gsm_show_statistics_outbound, "Displays statistics of outbound"),
};
#else  //(ASTERISK_VERSION_NUM > 10444)
static struct ast_cli_entry extra_gsm_cli[] = {
	{ { "gsm", "debug", "span", NULL },
	...
	{ { "gsm", "send", "sync", "sms",NULL },
	handle_gsm_send_sync_sms, "Send SMS and wait finish on a given GSM span",
	"Usage: gsm send sync sms <span> <destination> <message> <timeout> [id]\n"
	"       Send SMS and wait finish on a given GSM span\n", gsm_complete_span_5},

	{ { "gsm", "send", "sync", "csms",NULL },
	handle_gsm_send_sync_csms, "Send Concatenated SMS and wait finish on a given GSM span",
	"Usage: gsm send sync csms <span> <destination> <message> <flag> <smscount> <smssequence> <timeout> [id]\n"
	"       Send SMS Concatenated and wait finish on a given GSM span\n", gsm_complete_span_5},

	{ { "gsm", "send", "sync", "at",NULL },
	handle_gsm_send_sync_at, "Send AT Command and wait finish on a given GSM span",
	"Usage: gsm send sync at <span> <AT Command> <timeout(ms)>\n"
	...
	"Usage: gsm show debug at <span>\n"
	"       Show at command debug stat on a given GSM span\n", gsm_complete_span_5},
	{ { "gsm", "show", "statistics", "outbound", NULL },
	handle_gsm_show_statistics_outbound, "Displays statistics of outbound",
	"Usage: gsm show statistics outbound\n"
	"       Displays statistics of outbound\n", NULL },
...
//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
		ast_mutex_init(&gsms[i].txq_lock);
#endif //TX_QUEUE
...
#endif
		ast_mutex_init(&gsms[i].sms_lock);
		ast_mutex_init(&gsms[i].sms_mutex);
		ast_cond_init(&gsms[i].sms_cond,NULL);

		ast_mutex_init(&gsms[i].at_mutex);
		ast_cond_init(&gsms[i].at_cond,NULL);
...
#ifdef SIMULATE_DIAL
		gsms[i].simulatedial = 0;
#endif //SIMULATE_DIAL

		//Freedom Add for anonymous call 2013-08-15 09:37
		gsms[i].anonymouscall = 0;
		gsms[i].atz_timeout= 2;

		//Freedom Add for GSM band binding 2013-03-13 09:23 
		gsms[i].band[0] = 0;
		//Jason Add dial prefix 2013-5-9
		gsms[i].dialprefix[0] = 0;
	...
	for (span = 1; span < NUM_SPANS+1; ++span) {
		...
		if (s.alarms > 0) {
			if (s.alarms & DAHDI_ALARM_BLUE)
				strncat(alarmstr, "BLU/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (s.alarms & DAHDI_ALARM_YELLOW)
				strncat(alarmstr, "YEL/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (s.alarms & DAHDI_ALARM_RED)
				strncat(alarmstr, "RED/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (s.alarms & DAHDI_ALARM_LOOPBACK)
				strncat(alarmstr, "LB/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (s.alarms & DAHDI_ALARM_RECOVER)
				strncat(alarmstr, "REC/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (s.alarms & DAHDI_ALARM_NOTOPEN)
				strncat(alarmstr, "NOP/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (!strlen(alarmstr))
				strncat(alarmstr, "UUU/", sizeof(alarmstr)-strlen(alarmstr)-1);
			if (strlen(alarmstr)) {
				/* Strip trailing / */
			...
			if (s.numchans)
				...
				strncpy(alarmstr, "UNCONFIGURED", sizeof(alarmstr));
		}
...
#endif //(ASTERISK_VERSION_NUM > 10444)
{
	...
	strncpy(vi.version, "Unknown", sizeof(vi.version));
	strncpy(vi.echo_canceller, "Unknown", sizeof(vi.echo_canceller));
...
static int __unload_module(void)
{
	...
	for (i = 0; i < NUM_SPANS; i++) {
		...
		if (gsms[i].master != AST_PTHREADT_NULL) {
			...
			//ast_verb(0,"cancel thread master %d \n",i);
...
//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
		ast_mutex_destroy(&gsms[i].txq_lock);
#endif //TX_QUEUE

#ifdef CONFIG_CHECK_PHONE
		ast_mutex_destroy(&gsms[i].phone_lock);
		ast_mutex_destroy(&gsms[i].check_mutex);
		ast_cond_destroy(&gsms[i].check_cond);
#endif
		ast_mutex_destroy(&gsms[i].sms_lock);
		ast_mutex_destroy(&gsms[i].sms_mutex);
		ast_cond_destroy(&gsms[i].sms_cond);

		ast_mutex_destroy(&gsms[i].at_mutex);
		ast_cond_destroy(&gsms[i].at_cond);
		
		ast_mutex_destroy(&gsms[i].ussd_mutex);
		ast_cond_destroy(&gsms[i].ussd_cond);

	}
	//ast_verb(3,"unregister gsm cli\n");
	ast_cli_unregister_multiple(extra_gsm_cli, ARRAY_LEN(extra_gsm_cli));
#endif
	//ast_verb(3,"unregister extra cli\n");
	...
	//ast_verb(3,"unload chan extra sucess\n");
	return 0;
}
...
static void process_echocancel(struct extra_chan_conf *confp, const char *data, unsigned int line)
{
	...
	for (x = 1; x < param_count; x++) {
		...
		strncpy(confp->chan.echocancel.params[confp->chan.echocancel.head.param_count].name, param.name,sizeof(confp->chan.echocancel.params[confp->chan.echocancel.head.param_count].name));
...
static int process_extra(struct extra_chan_conf *confp, const char *cat, struct ast_variable *v, int reload, int options)
{
	...
	for (; v; v = v->next) {
		...
		////////////////////////////////////////////////////////////////
...
#ifdef SIMULATE_DIAL
			} else if (!strcasecmp(v->name, "simulatedial")) {
				confp->gsm.simulatedial = ast_true(v->value);
#endif //SIMULATE_DIAL
			//Freedom Add for anonymous call 2013-08-15 09:37
			} else if(!strcasecmp(v->name,"anonymouscall")) {
				confp->gsm.anonymouscall = ast_true(v->value);
			} else if (!strcasecmp(v->name, "atz_timeout")) {
				confp->gsm.atz_timeout = atoi(v->value);
			...
			//Freedom Add for GSM band binding 2013-03-13 09:23 
			/////////////////////////////////////////////////////
			} else if (!strcasecmp(v->name,"band")) {
				strncpy(confp->gsm.band, v->value, sizeof(confp->gsm.band)); 
			/////////////////////////////////////////////////////
			//Jason Add dial prefix 2013-5-9
			/////////////////////////////////////////////////////
			} else if(!strcasecmp(v->name,"dialprefix")){
				strncpy(confp->gsm.dialprefix, v->value, sizeof(confp->gsm.dialprefix));
			/////////////////////////////////////////////////////
...
#endif //(ASTERISK_VERSION_NUM >= 10800)
{
	...
	if((end-start) > sizeof(mesg) || (end-start) <= 0) {
		ast_log(LOG_WARNING, "%s message overflow\n",cmd);
		return -1;
	}

	strncpy(mesg,pdata,find-pdata);
	mesg[find-pdata]='\0';
	...
	if( strlen(mesg) <= 0 ) {
		ast_log(LOG_WARNING, "%s SMS message too short.\n",cmd);
		return -1;
	} else if( strlen(mesg) > GSM_MAX_SMS_LENGTH ) {
		ast_log(LOG_WARNING, "%s SMS message too long.\n",cmd);
		return -1;
	}	
	...
	if ( gsms[span_num-1].dchan ) {
		...
		if(!gsm_encode_pdu_ucs2(smsc,dest,mesg,char_coding,pdu,0,0,0,0)) {
...
static int load_module(void)
{
	...
	//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
	set_txq_lock_func(txq_lock,txq_unlock);
#endif
...
//Freedom Add for send AT commmands according to simple queue. 2013-07-09 14:13
#ifdef TX_QUEUE
		ast_mutex_init(&gsms[z].txq_lock);
#endif //TX_QUEUE

#ifdef CONFIG_CHECK_PHONE
		ast_mutex_init(&gsms[z].phone_lock);
		ast_mutex_init(&gsms[z].check_mutex);
		ast_cond_init(&gsms[z].check_cond,NULL);
#endif
		ast_mutex_init(&gsms[z].sms_lock);
		ast_mutex_init(&gsms[z].sms_mutex);
		ast_cond_init(&gsms[z].sms_cond,NULL);

		ast_mutex_init(&gsms[z].at_mutex);
		ast_cond_init(&gsms[z].at_cond,NULL);
		
		ast_mutex_init(&gsms[z].ussd_mutex);
		ast_cond_init(&gsms[z].ussd_cond,NULL);
