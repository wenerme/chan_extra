...
banner()
{
	...
	echo -e "#        Copyright (c) 2009-2013 OpenVox. All Rights Reserved.         #"
	echo -e "########################################################################"	
	echo ""
...
#		fi
	...
	echo "------------------------------------------"
	echo "n    : Download and install asterisk-1.8.9.0.tar.gz [Default installation]"
	...
	if [ "$response" == "n" ]; then
		cd /usr/src
		AST_SOURCE_DIR="asterisk-1.8.9.0"
...
get_dahdi_version()
{
	if [ -e $DAHDI_LINUX_COMPLETE_SOURCE_DIR/.version ]; then
		DAHDI_VER_STR=`cat $DAHDI_LINUX_COMPLETE_SOURCE_DIR/.version`
	elif [ -e $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/.version ]; then
		DAHDI_VER_STR=`cat $DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/.version`
	else
		echo "Can't get DAHDI version!"
		exit 1
	fi
...
install_opvxg4xx()
{
	...
	if [ x"$VIRTUAL_TTY" == x"YES" ]; then
		clearscr
		banner
        
		echo
		logger "You can choose some spans as tty device. First choose tty mode ...\n"
		echo 
		echo -e "1 : Normal Mode. \tSet spans to normal tty mode for data service"
		echo -e "2 : Mux Mode. \t\tSet spans to mux tty mode for data service and voice communication at the same time"
		echo -e "3 : None"
		echo "Notice that Mux mode is slightly unstable for some 3G modules."
		echo "(ctrl-c to Exit)"
		echo 
		echo -n "Please select ttymode(Enter for default Mux): "
		read ttymode
		logger "Please select ttymode: $ttymode\n" 0
	...
	if [ x"$VIRTUAL_TTY" == x"NORMAL" ]; then
		echo 
		logger "Choose spans for Normal tty mode ...\n"
		echo 
		echo "There are 4 spans for every opvxg4xx card. "
		echo "If you have one opvxg4xx card, you can choose id 1~4. eg. normal ttymode: 3,4"
		echo "If you have two opvxg4xx cards, you can choose id 1~8. eg. normal ttymode: 3,7,8"
		echo "Span id Scope: 1~255"
		echo "Notice that the span set to normal tty mode cannot be used for voice communication. "
		echo "(ctrl-c to Exit)"
		echo 
		echo "Please input span ids for normal ttymode "
		echo -n "(Separated by a comma, Enter for default none): "
		read ttymode_normal
		logger "Please input span ids for normal ttymode: $ttymode_normal\n" 0

		str_normal=`echo "$ttymode_normal," | grep '^\([0-9]\+,\)\+$'`
		if [ "$str_normal" == "" ];then
			echo
			echo -n "Normal TTY Mode Spans: none. "
		else
			#str_len=`echo "$ttymode_normal" | awk '{print split($0,arr,",");}'`
			str_len=`echo "$ttymode_normal" | awk '{len=split($0,arr,",");ret=len;for(i=1;i<=len;i++){if(arr[i]<1||arr[i]>255){ret=0;}};print ret;}'`
			if [ "$str_len" != "0" ];then
				TTYMODE_NORMAL=$ttymode_normal
				TTYMODE_LEN=$str_len
			else
				install_opvxg4xx
				return 0
			...
			echo
			echo -n "Normal TTY Mode Spans: $TTYMODE_NORMAL. "
		fi

		if [ "$TTYMODE_NORMAL" == "" ];then
			TTYMODE_NORMAL=0
		fi
		if [ "$TTYMODE_LEN" == "" ];then
			TTYMODE_LEN=0
		...
		echo -n "Please input Enter to continue"
		read enter
	fi

	if [ x"$VIRTUAL_TTY" == x"MUX" ]; then
		echo 
		logger "Choose spans for MUX tty mode ...\n"
		echo 
		echo "There are 4 spans for every opvxg4xx card for you to set to MUX mode ."
		echo "If you have one opvxg4xx card, you can choose id 1~4. eg. MUX ttymode: 3,4"
		echo "If you have two opvxg4xx cards, you can choose id 1~8. eg. MUX ttymode: 3,7,8"
		echo "Span id scope : 1~255"
		echo "(ctrl-c to Exit)"
		echo 
		echo "Please input span ids for MUX ttymode "
		echo -n "(Separated by a comma, Enter for default none): "
		read ttymode_mux
		logger "Please input span ids for MUX ttymode: $ttymode_mux\n" 0

		str_mux=`echo "$ttymode_mux," | grep '^\([0-9]\+,\)\+$'`
		if [ "$str_mux" == "" ];then
			echo
			echo -n "MUX TTY Mode Spans: none. "
		else
			#str_len=`echo "$ttymode_normal" | awk '{print split($0,arr,",");}'`
			str_len=`echo "$ttymode_mux" | awk '{len=split($0,arr,",");ret=len;for(i=1;i<=len;i++){if(arr[i]<1||arr[i]>255){ret=0;}};print ret;}'`
			if [ "$str_len" != "0" ];then
				TTYMODE_MUX=$ttymode_mux
				TTYMODE_LEN=$str_len
			else
				install_opvxg4xx
				return 0
			...
			echo
			echo -n "MUX TTY Mode Spans: $TTYMODE_MUX. "
		fi
	...
	######################################################################
	get_dahdi_version
	if [ x"$DAHDI_VER_NUM" == x"0000" ] || [ "$DAHDI_VER_NUM" == "" ]; then
		error "get dahdi version error\n"
		exit 1
	fi
	OPVXG4XX_H_FILE="$DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/opvxg4xx/opvxg4xx.h"
	BASE_C_FILE="$DAHDI_LINUX_COMPLETE_SOURCE_DIR/linux/drivers/dahdi/opvxg4xx/base.c"
	if [ x"$VIRTUAL_TTY" == x"NORMAL" ]; then
		sed -i "/#define _OPVXG4XX_H_/a#define DAHDI_VER_NUM $DAHDI_VER_NUM\n#define VIRTUAL_TTY" $OPVXG4XX_H_FILE
		sed -i "s/static int ttymode\[256\] = {0};/static int ttymode\[256\] = {$TTYMODE_NORMAL};/" $BASE_C_FILE
		sed -i "s/static int ttymode_len;/static int ttymode_len = $TTYMODE_LEN;/" $BASE_C_FILE
		eval $(echo "$TTYMODE_NORMAL" | awk '{split($0,arr,",");for(i in arr){print "arr["i"]="arr[i];}}')
		condition=0
		for i in ${arr[*]}
		do
			condition="$condition || \$span->num==$i"
		done
		sed -i "/\$self->gen_digital(\$gconfig, \$span);/i\\\t\t\t\tif($condition){next;}" $DAHDI_LINUX_COMPLETE_SOURCE_DIR/tools/xpp/perl_modules/Dahdi/Config/Gen/Chanextra.pm
	elif [ x"$VIRTUAL_TTY" == x"MUX" ]; then
		sed -i "/#define _OPVXG4XX_H_/a#define DAHDI_VER_NUM $DAHDI_VER_NUM\n#define VIRTUAL_TTY\n#define GSM0710_MODE" $OPVXG4XX_H_FILE
		condition=0
		eval $(echo "$TTYMODE_MUX" | awk '{split($0,arr,",");for(i in arr){print "arr["i"]="arr[i];}}')
		for i in ${arr[*]}
		do
			condition="$condition || \$num==$i"
		done
		sed -i "/printf \";tty=off\\\n\";/i\\\tif($condition){printf \"tty=on\\\n\";}" $DAHDI_LINUX_COMPLETE_SOURCE_DIR/tools/xpp/perl_modules/Dahdi/Config/Gen/Chanextra.pm
	else
...
install_libgsmat()
{
	...
	grep '#define GSM0710_MODE' $LIBGSMAT_H_FILE_PATH > /dev/null 2>&1
	if [ $? -ne 0 ]; then
		if [ x"$VIRTUAL_TTY" == x"MUX" ]; then
			sed -i "/#define _LIBGSMAT_H/a#define GSM0710_MODE" $LIBGSMAT_H_FILE_PATH
		fi
	else
		if [ "$VIRTUAL_TTY" != x"MUX" ]; then
			sed -i "/#define GSM0710_MODE/d" $LIBGSMAT_H_FILE_PATH
		fi
	fi
...
usage()
{
	...
	echo "tty=normal|mux|yes  -TTY mode normal(/dev/ttyGSM*) or multiplexing(Asterisk and /dev/ttyGSM* share at same time) or choose inside"
	echo
}
#############################################################################
